...
def load_or_create_plan(
    ...
    expected_criteria = criteria.to_plan_dict() if criteria else None
    if path.exists() and not fresh:
        plan = json.loads(path.read_text(encoding="utf-8"))
        if criteria is not None and plan.get("criteria") != expected_criteria:
    ...
    return {
        ...
        "pending": [[surname] for surname in surnames],
        "completed": [],
        "unresolved": [],
        "active": None,
    }
